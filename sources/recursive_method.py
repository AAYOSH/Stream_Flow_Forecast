...
def avaliar_modelo(previsao,esperado):
...
def recursive_method(train,test,n_input,n_output,epochs,batch_size,n_units):
    ...
    ## modelo é construido de forma a prever apenas um passo a frente
    modelo = build_model(train_scaled,test_scaled,n_input,n_output,epochs,batch_size,n_units)
...
# ====================================================================
# train dataset
    modelo_rec,historico = recursive_method(train,test,n_input,n_output,epochs,batch_size,n_units)
    train = np.array(train).reshape(-1,1)
    scaler = MinMaxScaler(feature_range=(0, 1))
    train_scaled = scaler.fit_transform(train)
    ...
    for sample in X:
        y_hat = np.array(series_forecast(modelo_rec,sample,7))
        y_hat = scaler.inverse_transform(y_hat.reshape(-1,1))
        Y_hat.append(y_hat)
        
    Y = scaler.inverse_transform(Y)
    Y_hat = np.array(Y_hat)[:,:,0]
    score_,scores_ = avaliar_modelo(Y_hat,Y)

    print(f"Performance train set recursive method {score_,scores_}")
...
# test dataset
    ...
    scaler = MinMaxScaler(feature_range=(0, 1))
    test_scaled = scaler.fit_transform(test)
    X,Y = to_supervised(test_scaled,25,7)

    Y_hat = []
    for sample in X:
        y_hat = np.array(series_forecast(modelo_rec,sample,25))
        y_hat = scaler.inverse_transform(y_hat.reshape(-1,1))
        Y_hat.append(y_hat)
    ...
    print(f"Performance test set recursive method {score_,scores_}")